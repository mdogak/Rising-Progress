...
(function(){
  ...
  function applyTimeseriesProjectMeta(model, tsProjectMeta, mode){
    ...
    if (!model.timeSeriesProject || typeof model.timeSeriesProject !== 'object') model.timeSeriesProject = {};
    ...
    for (var i=0;i<n;i++){
      var hd = String(hdates[i] || '');
      if (!hd) continue;

      if (!model.timeSeriesProject[hd]) model.timeSeriesProject[hd] = [];
      model.timeSeriesProject[hd].push({
        historyDate: hd,
        key: String(keys[i] || ''),
   ...
   * 1) Load JSON containing timeseries.projectMeta; confirm no runtime errors.
   * 2) In console:
   *    Object.keys(getModel().timeSeriesProject || {}).length > 0
   *    getModel().timeSeriesProject['2025-11-27'] -> array of {historyDate,key,value}
   * 3) Confirm existing timeseries sections still load identically (curves, scope/section snapshots).
   */
  ...
  window.PRGSJSON_LOADER = {
    load: function(jsonInput, opts){
      ...
      if (ts){
        ...
        if (ts.projectMeta) applyTimeseriesProjectMeta(model, ts.projectMeta, mode);
