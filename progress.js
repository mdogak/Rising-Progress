...
function loadFromXml(text) {
    ...
    model = {
        ...
        history: [],
        dailyActuals: {},
        baseline: { days: [], planned: [] }
    };

    // Project name
    const nameNode = xmlDoc.getElementsByTagName("Name")[0];
    if (nameNode) model.project.name = nameNode.textContent.trim();

    // Extended attributes
    const ext = xmlDoc.getElementsByTagName("ExtendedAttribute");
    for (let i = 0; i < ext.length; i++) {
        const nameTag = ext[i].getElementsByTagName("Name")[0];
        const valueTag = ext[i].getElementsByTagName("Value")[0];
        if (!nameTag || !valueTag) continue;

        const tagName = nameTag.textContent.trim();
        const raw = valueTag.textContent.trim();

        switch (tagName) {
            case "Startup":
                model.project.startup = raw;
                break;
            case "MarkerLabel":
                model.project.markerLabel = raw;
                break;

            case "LegendBaselineCheckbox":
                document.getElementById("legend-baseline").checked = (raw === "true");
                break;
            case "LegendPlannedCheckbox":
                document.getElementById("legend-planned").checked = (raw === "true");
                break;
            case "LegendActualCheckbox":
                document.getElementById("legend-actual").checked = (raw === "true");
                break;
            ...
            case "BaselineHistory":
                ...
                raw.split("/\r?\n/").forEach(line => {
                    ...
                    if (parts.length >= 2) {
                        model.baseline.days.push(parts[0]);
                        model.baseline.planned.push(parseFloat(parts[1]));
                    }
                });
                break;
            ...
            case "DailyActuals":
                ...
                raw.split("/\r?\n/").forEach(line => {
                    ...
                    const parts = line.split(",");
                    if (parts.length >= 2) {
                        model.dailyActuals[parts[0]] = parseFloat(parts[1]);
                    }
                });
                break;
        }
    ...
    for (let i = 0; i < tasks.length; i++) {
        const t = tasks[i];
        const get = t.getElementsByTagName.bind(t);

        const label = (get("Name")[0]?.textContent || "").trim();
        const start = (get("Start")[0]?.textContent || "").substring(0, 10);
        const finish = (get("Finish")[0]?.textContent || "").substring(0, 10);
        const pct = parseFloat(get("PercentComplete")[0]?.textContent || "0");
        const cost = parseFloat(get("Cost")[0]?.textContent || "0");
        const prog = parseFloat(get("ProgressValue")[0]?.textContent || "0");
        const totalUnits = (get("TotalUnits")[0]?.textContent || "").trim();
        const unitsLabel = (get("UnitsLabel")[0]?.textContent || "").trim();
        ...
        model.scopes.push({
            ...
            cost,
            unitsToDate: prog,
            totalUnits: totalUnits === "" ? "" : parseFloat(totalUnits),
    ...
    document.getElementById("startupLabelInput").value = model.project.markerLabel;
