...
function defaultScope(i){
  if(i===0){ const startDate = new Date(today); startDate.setDate(startDate.getDate()-1); const endDate = new Date(startDate); endDate.setDate(endDate.getDate()+7); const start = fmtDate(startDate); const end = fmtDate(endDate); return { scopeId: uid, label:`Scope #${i+1}`, start, end, cost:100, actualPct:0, unitsToDate:0, totalUnits:'', unitsLabel:'%', sectionName:'' }; }
...
// Save/Load/Export logic moved to js/save-load.js (ES module)
...
initSaveLoad({
  ...
  // Session key + hydration (used by auto-load default)
  COOKIE_KEY,
  hydrateFromSession,

  // Legend state bridge
  getLegendState: ()=>({ baselineVisible, plannedVisible, actualVisible, forecastVisible }),
  setLegendState: (patch)=> {
    if (patch && typeof patch === 'object') {
      if ('baselineVisible' in patch) baselineVisible = !!patch.baselineVisible;
      if ('plannedVisible' in patch) plannedVisible = !!patch.plannedVisible;
