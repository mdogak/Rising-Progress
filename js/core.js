...
export function getLocalToday() { const now = new Date(); return new Date(now.getFullYear(), now.getMonth(), now.getDate()); }
export const today = getLocalToday();
export function parseDate(val){ return val ? new Date(val + 'T00:00:00') : null }
export function fmtDate(d){ return d ? d.toISOString().slice(0,10) : '' }
export function fmtLongDateStr(dStr){ const d=parseDate(dStr); return d? d.toLocaleDateString(undefined,{year:'numeric',month:'long',day:'numeric'}) : dStr }
export function fmtLongToday(){ return new Date().toLocaleDateString(undefined,{year:'numeric',month:'long',day:'numeric'}) }
export function daysBetween(a,b){ const ms = (parseDate(fmtDate(b)) - parseDate(fmtDate(a))); return Math.floor(ms/86400000)+1; }
export function clamp(n,min,max){ return Math.max(min, Math.min(max,n)) }

// Cookie helpers
export function setCookie(name, value, days=365){ const d = new Date(); d.setTime(d.getTime() + (days*24*60*60*1000)); const v = encodeURIComponent(value); document.cookie = `${name}=${v};expires=${d.toUTCString()};path=/`; }
export function getCookie(name){ const n = name + '='; const ca = document.cookie.split(';'); for(let c of ca){ while(c.charAt(0)==' ') c = c.substring(1); if(c.indexOf(n)==0) return decodeURIComponent(c.substring(n.length,c.length)); } return null; }
export function delCookie(name){ document.cookie = `${name}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;` }
...
export function defaultScope(i){
...
export function ensureRows(n){ const cont = $('#scopeRows'); const cur = cont.children.length; for(let i=cur;i<n;i++) cont.appendChild(renderScopeRow(i)); }
export function syncScopeRowsToModel(){ const cont = $('#scopeRows'); cont.innerHTML = ''; for(let i=0;i<model.scopes.length;i++) cont.appendChild(renderScopeRow(i)); }

export function renderScopeRow(i){
...
export function onScopeChange(e){
...
export function calcScopePlannedPctToDate(s){
...
export function updatePlannedCell(row, s){ const plannedPct = calcScopePlannedPctToDate(s); const cell = row.querySelector('[data-k="planned"]'); if(s.totalUnits!=='' && Number(s.totalUnits)>0){ const plannedUnits = (plannedPct/100) * Number(s.totalUnits); cell.textContent = plannedUnits.toFixed(1); } else { cell.textContent = plannedPct.toFixed(1)+'%'; }
...
export function calcScopeWeightings(){ const total = model.scopes.reduce((a,b)=>a+(b.cost||0),0) || 0; return model.scopes.map(s=> total>0 ? (s.cost/total) : 0); }
export function calcPlannedDailyOverall(){
...
export function calcTotalActualProgress(){ const weightings = calcScopeWeightings(); const total = model.scopes.reduce((sum, s, i)=> sum + (weightings[i]||0) * (isFinite(s.actualPct)?s.actualPct:0), 0); return clamp(total,0,100); }
...
export function buildFallbackRange(){ const d = fmtDate(today); return {days:[d], planned:[0], actual:[0]}; }
export function lastActualDate(){ let last=null; for(const k of Object.keys(model.dailyActuals)){ if(model.dailyActuals[k]!=null){ const d=parseDate(k); if(!last||d>last) last=d; } } model.history.forEach(h=>{ const d=parseDate(h.date); if(!last||d>last) last=d; }); const ps = calcEarliestStart(); if(!last && ps) last = ps; return last; }
export function calcPlannedSeriesByDay(){ const days = buildDateRange(); if(days.length===0){ const f=buildFallbackRange(); return {days:f.days, plannedCum:f.planned}; } const per = calcPlannedDailyOverall(); const plannedCum=[]; let cum=0; for(const ds of days){ const d=parseDate(ds); let add=0; per.forEach(p=>{ if(p.start && p.end && d>=p.start && d<=p.end) add += p.perDay; }); cum += add; plannedCum.push(clamp(cum,0,100)); } return {days, plannedCum}; }
export function calcActualSeriesByDay(days){ if(!days || days.length===0){ const f=buildFallbackRange(); return f.actual; } const known = new Map(); const ps = calcEarliestStart(); if(ps){ const pre = new Date(ps); pre.setDate(pre.getDate()-1); known.set(fmtDate(pre), 0); } for(const [d,v] of Object.entries(model.dailyActuals)){ if(v!=null) known.set(d, clamp(Number(v)||0,0,100)); } model.history.forEach(h=> known.set(h.date, clamp(Number(h.actualPct)||0,0,100)));
...
export function takeBaseline(days, plannedCum){ model.baseline = { days: days.slice(), planned: plannedCum.slice(), ts: Date.now() }; setCookie(COOKIE_KEY, JSON.stringify(model), 3650); }

/*****************
 * Rendering & Chart
 *****************/
export let chart;
export let baselineVisible = true;
export let legendStats = {baselinePct:null, plannedPct:null, actualPct:null, daysRelText:''};
export let plannedVisible = true;
export let actualVisible = true;

export function updateBelowChartStats(days, baselineCum, plannedCum, actualCum){
...
export function refreshLegendNow(){
  try{
...
export function drawChart(days, baseline, planned, actual){
...
export function renderDailyTable(days, baseline, planned, actual){
...
export function csvEsc(v){ if(v==null) return ''; const s = String(v); return /[",\n]/.test(s) ? '"'+s.replace(/"/g,'""')+'"' : s; }
export function csvLine(arr){ return arr.map(csvEsc).join(',') + '\n'; }

export function buildAllCSV(){
...
export function uploadCSVAndLoad(){ const inp = document.createElement('input'); inp.type='file'; inp.accept='.csv,text/csv,application/xml,.xml'; inp.onchange = () => { const file = inp.files[0]; if(!file) return; const reader = new FileReader(); reader.onload = (e)=>{ try{ const text = e.target.result;
 ...
 *****************/
export const COOKIE_KEY='progress_tracker_v3b';
...
export function parseAndLoadFullCSV(text){
